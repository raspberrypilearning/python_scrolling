...
If your game is correctly detecting collisons, then check that you have properly indented the code that draws your game inside the `if level > 0` satement, to make sure it only runs if that statement is true. उदाहरणार्थ:
...
Finally, if both of those are working correctly, your game may not be setting `level = 0` correctly when a collision happens. उदाहरणार्थ:
...
If level is increasing correctly, check your `draw_obstacles()` function. In particular, check that you have `ob_y = randint(0, height) + (frame_count * level)`. परिणाम यासारखे दिसेल:
...
If level is increasing correctly, check your `draw_obstacles()` function to ensure that you have `level` used in the `range()` function of the `for` loop that draws the obstacles. परिणाम यासारखे दिसेल:
