...
def desenhar_obstaculos():
    ob_x = width/2
    ob_y = height/2
    text('🌵', ob_x, ob_y)  # Substitua pelo seu obstáculo
...
def draw():
    ...
    background(seguro)
    desenhar_obstaculos()  # Antes de desenhar o jogador
    desenhar_jogador()
...
def setup():
    size(400, 400)
    global jogador
    jogador = load_image('turtle.png')  # Carrega sua imagem de jogador
    obstaculo = load_image('shark.png')  # Carrega sua imagem de obstáculo
...
def desenhar_obstaculos():
    ...
    image(obstaculo, ob_x, ob_y, 30, 30)  # Redimensione para adequar ao seu tema
...
--- collapse ---
---
title: Desenhe um obstáculo usando várias formas
---
...
def desenhar_obstaculos():
    ...
    # Desenhe um pinheiro
    no_stroke()
    fill(0,255,0)  # Verde para as folhas
    ...
    fill(150,100,100)  # Marrom para o tronco
...
def desenhar_obstaculos():
    ob_x = width/2
    ob_y = height/2 + frame_count  # Aumenta a cada quadro
...
def desenhar_obstaculos():
    seed(12345678)  # Está bem qualquer número
...
--- collapse ---
---
title: Os obstáculos mudam de posição toda vez que um quadro é desenhado
---
